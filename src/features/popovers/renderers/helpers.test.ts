...
describe('extractUrl', () => {
	...
	it('extracts URL from markdown link', () => {
		expect(extractUrl('[Example](https://example.com)')).toBe(
			'https://example.com',
		);
	});
	...
	it('extracts URL from markdown link with descriptive text', () => {
		expect(extractUrl('[My Site](https://mysite.org/page)')).toBe(
			'https://mysite.org/page',
		);
	});
});
...
describe('cleanAllValues', () => {
	...
	it('cleans an array of values', () => {
		expect(cleanAllValues(['[[React]]', '[[Node]]'])).toEqual([
			'React',
			'Node',
		]);
	});
