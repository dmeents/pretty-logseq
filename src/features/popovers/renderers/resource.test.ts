...
describe('Resource Renderer', () => {
	...
	describe('render', () => {
		...
		it('renders property tags', () => {
			...
			expect(Array.from(tags).map((t) => t.textContent)).toEqual(['Resource', 'Active', 'Development']);
		});
		...
		it('renders complete resource card', () => {
			...
			expect(result.querySelector('.pretty-popover__title')?.textContent).toBe('🚀 GraphQL API Guide');
