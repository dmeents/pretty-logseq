...
describe('renderPopover', () => {
	...
	describe('header', () => {
		...
		it('renders photo card when photo property exists', () => {
			const el = renderPopover(
				makePage('Person', { photo: 'https://example.com/photo.jpg' }),
			);
		...
		it('renders title inside photo card header info', () => {
			const el = renderPopover(
				makePage('John', { photo: 'https://example.com/photo.jpg' }),
			);
	...
	describe('subtitle', () => {
		...
		it('renders combined role at organization subtitle', () => {
			const el = renderPopover(
				makePage('Person', { role: 'CTO', organization: 'Startup Inc' }),
			);
			const subtitle = el.querySelector('.pretty-popover__subtitle');
	...
	describe('description', () => {
		it('renders description when present', () => {
			const el = renderPopover(
				makePage('Page', { description: 'A useful page' }),
			);
	...
	describe('snippet', () => {
		it('renders snippet when page has no rich properties', () => {
			const el = renderPopover(
				makePage('Page', { type: 'Note' }, [
					{ content: 'Some block content here' },
				]),
			);
		...
		it('does not render snippet when page has detail properties', () => {
			const el = renderPopover(
				makePage('Page', { rating: 4 }, [{ content: 'Block content' }]),
			);
		...
		it('does not render snippet when page has url', () => {
			const el = renderPopover(
				makePage('Page', { url: 'https://example.com' }, [
					{ content: 'Content' },
				]),
			);
			expect(el.querySelector('.pretty-popover__snippet')).toBeNull();
	...
	describe('detail rows', () => {
		it('renders detail rows for detail properties', () => {
			const el = renderPopover(
				makePage('Page', { rating: 4, location: 'NYC' }),
			);
		...
		it('renders email as mailto link in detail row', () => {
			const el = renderPopover(
				makePage('Page', { email: 'test@example.com' }),
			);
			const link = el.querySelector(
				'.pretty-popover__detail-link',
	...
	describe('array properties', () => {
		it('renders array properties as tag pills', () => {
			const el = renderPopover(
				makePage('Page', { stack: ['React', 'Node', 'TS'] }),
			);
	...
	describe('link section', () => {
		it('renders link section when url property exists', () => {
			const el = renderPopover(
				makePage('Page', { url: 'https://github.com/user/repo' }),
			);
	...
	describe('tags', () => {
		...
		it('includes extra tags like relationship', () => {
			const el = renderPopover(
				makePage('Page', { type: 'Person', relationship: 'Friend' }),
			);
	...
	describe('full page scenarios', () => {
		...
		it('renders a rich person page', () => {
			...
			// Photo card header
			expect(
				el.querySelector('.pretty-popover__photo-card'),
			).not.toBeNull();
			expect(el.querySelector('.pretty-popover__title')?.textContent).toBe(
				'👩 Alice',
			);

			// Subtitle
			expect(
				el.querySelector('.pretty-popover__subtitle')?.textContent,
			).toBe('Designer at Design Co');

			// Description
			expect(
				el.querySelector('.pretty-popover__description')?.textContent,
			).toBe('UI designer');

			// Detail rows (email)
			expect(
				el.querySelector('.pretty-popover__detail-link'),
			).not.toBeNull();
		...
		it('renders a codebase page with repository link', () => {
			...
			const repoLink = el.querySelector(
				'.pretty-popover__detail-link',
			) as HTMLAnchorElement;
